...
#ifndef HELIOS_PART_WORKLOAD_H
#define HELIOS_PART_WORKLOAD_H
...
public:
    ...
    int init();
    template <class T>
    int reset_lubm_query_bank(T &QB_worload,T &QB);
private:
    int set_file_path();
    int set_direction();

    template <class T>
    int add_query_to_query_bank(T &QB_worload,T & QB,std::string s,int workload_index,int pattern_index,int index) const;
 ...
 */
template <class T>
int Workload_T::reset_lubm_query_bank(T &QB_worload,T &QB) 
{
    ...
    std::cout << "QB_worload.Delete finish" << std::endl;
	...
	if (pattern_index >= vec_replace_instance.size())
	{
        ...
        {
             //std::cout <<"22=" << pattern_index << std::endl;
             ...
             // std::cout << "1" << std::endl;
              ...
              // std::cout << "3" << std::endl;
            //   std::cout << "subject or object=" <<vec_replace_instance[pattern_index].at(vec_tmp[k_rand]).substr(0,1) << std::endl;
 ...
 */
template <class T>
int Workload_T::add_query_to_query_bank(T &QB_worload,T &QB,std::string s,int workload_index,int pattern_index,int index) const
{
