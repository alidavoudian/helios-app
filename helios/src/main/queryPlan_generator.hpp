		...
		void determine_bind_val_for_joint_children(QueryNode *sNode, int sNode_resultIndex) {
			...
			for (int e = 0; e < sNode->edges.size(); e++) {
				...
				if (tNode->edges.size() != 0) { 
					...
					/////////////////
					// Determine <tNode->bind_val> based on those <sNode>'s results whose predicates belongs to 
					// <sNode->edges[e]->bind_val>
					for (int i = 0; i < qpr.result_map[sNode_resultIndex].size(); i++) {
				
						/////////////////
						found = binary_search(sNode->edges[e]->bind_val.begin(), sNode->edges[e]->bind_val.end(),
											  qpr.result_map[sNode_resultIndex][i].p);
						if (found)					
							/////////////////
							tNode->bind_val.insert(tNode->bind_val.end(), 
												   qpr.result_map[sNode_resultIndex][i].o_vals.begin(),
												   qpr.result_map[sNode_resultIndex][i].o_vals.end());			   
					}
