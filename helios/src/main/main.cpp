...
void* repartitioner_thread(void *arg){
	Repartitioner *repartitioner = (Repartitioner *)arg;
	repartitioner->run();
}

void* up_edge_weight_thread(void *arg){
	Repartitioner *repartitioner = (Repartitioner *)arg;
...
if(argc == 4){
	...
	for(int tid = 0; tid < num_loader_threads; tid++){
		...
		pthread_create(&(loader_threads[tid]), NULL, load_thread, (void*)loader);
	...
	printf("%d data loading time: %9.7f\n",world_rank , MPI_Wtime() - begin);
}else{
	////////////////////////////////
	...
	boost::thread query_p = boost::thread(boost::bind(&query_proxy, world_rank, &config, mem));
	boost::this_thread::sleep( boost::posix_time::milliseconds(10) );
	
	boost::thread queryPlan_p = boost::thread(boost::bind(&queryPlan_proxy, world_rank, &config, mem));
	boost::this_thread::sleep( boost::posix_time::milliseconds(10) );
	
	boost::thread subQueryPlan_p = boost::thread(boost::bind(&subQueryPlan_proxy, world_rank, &config, mem));
...
if(config.enable_reassign){
	...
	pthread_create(&(repart_threads[0]), NULL, repartitioner_thread, (void*)repart);
	...
	for(int i = 1; i < num_repart_threads; i++){
		...
		pthread_create(&(repart_threads[i]), NULL, up_edge_weight_thread, (void*)repart2);
	}
/*	boost::thread metaDataHandler_p = boost::thread(boost::bind(&metaDataHandler_proxy, 
	...
	////////////////////////////////
	// Create and run client threads whose number is equal to numberOfClients
	if (world_rank == 0) {
	/*	int n; //For gdb in the distributed mode
		std::cout << "Enter n: "; std::cin >> n;*/
		for (int client_nbr = 1; client_nbr <= numberOfClients; client_nbr++)
			clientThread_list.push_back(boost::thread(boost::bind(&client_task, current_nodeName, 
	                                    numberOfQueries, client_nbr,&config)));
